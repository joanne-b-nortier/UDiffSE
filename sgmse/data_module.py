...
def getTIMITclean(subset, data_dir="file/path/to/TCD-TIMIT/", t='_data_NTCD'):
    if subset == 'valid':
        subset = 'val'
    t1 = subset + t
    if subset == 'test':
...
def getTIMITnoisy(subset, data_dir="file/path/to/NTCD-TIMIT-noisy/", t='Babble/15/volunteers'):
    if subset=='train':
        exclude_folders = ['06M', '09F', '17F',  '20M', '24M', '26M', '27M', '33F',  '35M',  '38F', '40F', '42M', '47M', '49F', '52M' , '56M', '59F']
        noisy_files = (sorted([os.path.join(root, name) for root, dirs, files in os.walk(os.path.join(data_dir, t)) for name in files if (name.endswith('.wav') and not any(exclude_folder in root for exclude_folder in exclude_folders) )])  )
    elif subset=='valid':
        include_folders = ['06M',  '17F',  '20M',  '35M', '38F', '42M', '52M', '59F']
        noisy_files = (sorted([os.path.join(root, name) for root, dirs, files in os.walk(os.path.join(data_dir, t)) for name in files if (name.endswith('.wav') and any(include_folder in root for include_folder in include_folders) )])  )
    elif subset=='test':
        include_folders = ['09F',  '24M',  '26M',  '27M',  '33F',  '40F',  '47M',  '49F', '56M']
        noisy_files = (sorted([os.path.join(root, name) for root, dirs, files in os.walk(os.path.join(data_dir, t)) for name in files if (name.endswith('.wav') and any(include_folder in root for include_folder in include_folders) )])  )
    else:
...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            format='default', normalize="clean", spec_transform=None,
        ...
        if format == "default":
            ...
            data_dir="file/path/to/TCD-TIMIT/" 
            t='_data_NTCD'
            self.clean_files = getTIMITclean(subset, data_dir, t)

        elif format == "wsj0":
            data_dir = "file/path/to/wsj0_wav"
            dic = {'train': '**/si_tr_s/**/*.wav', 
                   'valid': '**/si_dt_05/**/*.wav', 
                   'test': '**/si_et_05/**/*.wav'}
    ...
    def __getitem__(self, i):
        ...
        # normalize w.r.t to the clean signal or not at all
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--format", type=str, choices=("default","tcd-timit", "dns", "wsj0"), required=True, help="Read file paths according to file naming format.")
    ...
    def __init__(
        self, base_dir, format='wsj0', batch_size=8,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_set = Specs(data_dir=self.base_dir, subset='train',
                ...
                spectogram_learning=self.spectogram_learning, **specs_kwargs)
