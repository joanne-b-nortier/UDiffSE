...
class OUVESDE(SDE):
    ...
    def prior_sampling(self, shape, device):
        ...
        x_T = (torch.randn(shape, dtype=torch.complex64) * std[:, None, None, None]).to(device)
        return x_T
...
class VPSDE(SDE):
    ...
    def __init__(self, beta_min=0.1, beta_max=20, N=1000, **ignored_kwargs):
        ...
        self.beta_tilde = self.discrete_betas * ((self.alphas-self.alphas_cumprod)/(self.alphas*(1-self.alphas_cumprod)))
        self.__name__ = 'vpsde'
...
class OUVPSDE(SDE):
    ...
    def sde(self, x, t):
        drift = 0.5 * self.stiffness * batch_broadcast(self._beta(t), x) * ( - x) # $changed batch_broadcast to x.shape which should be the same
